...
/******************* global methods *******************/
/*
<tomalak> the first parameter to an operator is always the left operand,
and the second parameter is always the right operand.
and for an operator member the first parameter is IMPLICIT (means you can't see it, but it's there) and is *this.

<tomalak> things would break if you had C++ just fucking magically guessing at what operator you wanted to use

<Artifice> Also, always take a const ref as your roperand (damn my imaginary dyslexia)
*/
...
ostream &operator<<(ostream& output, const Matrix4x4 mat) {
	//output << "(" <<  p.x << ", " << p.y <<")";
